  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 space-y-20">
      <AnimatedSection className="space-y-6">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground max-w-3xl leading-relaxed text-sm md:text-base">
            I&apos;m an Independent Software Developer specializing in backend
            and full-stack systems. I have experience building scalable,
            event-driven architectures, secure authentication flows, and
            real-time applications. I am passionate about writing modular,
            maintainable code, optimizing database performance, and designing
            robust APIs.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {ABOUT_VALUE_CARDS.map((c) => {
            return (
              <div
                key={c.title}
                className="relative overflow-hidden rounded-xl border bg-card/60 p-5 backdrop-blur-sm group"
              >
                <div className="flex items-start gap-3">
                  <span className="rounded-md bg-primary/10 p-2 ring-1 ring-primary/20 text-primary group-hover:scale-110 transition-transform flex items-center justify-center">
                    {getIcon(c.icon, 'size-5')}
                  </span>
      ...
      <AnimatedSection className="space-y-10">
        <h2 className="font-semibold text-xl flex items-center gap-2">
          <Cpu className="size-4 text-primary" /> Technologies Snapshot
        </h2>
        <div className="flex flex-wrap gap-2">
          {SKILLS.slice(0, 12).map((s) => (
            <span
              key={s.name}
              className="rounded-md border bg-muted/40 px-2 py-1 text-[11px] font-medium tracking-wide hover:bg-muted transition-colors"
            >
              {s.name}
            </span>
          ))}
        ...
        <p className="text-xs text-muted-foreground max-w-3xl">
          Curious about the full list? Visit the dedicated skills page for
          interactive filtering and proficiency levels.
        </p>
      </AnimatedSection>

      <AnimatedSection className="space-y-6">
        <h2 className="font-semibold text-xl flex items-center gap-2">
          <Compass className="size-4 text-primary" /> Current Interests
        </h2>
        <div className="grid gap-4 md:grid-cols-2">
          {ABOUT_INTERESTS.map((i) => (
            <div
              key={i.title}
              className="rounded-lg border p-5 bg-card/50 hover:border-primary/50 transition-colors"
            >
